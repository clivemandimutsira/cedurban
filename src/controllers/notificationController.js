...
exports.createNotification = async (req, res) => {
  ...
  try {
    ...
    const recipients = await getRecipientIds(payload);
    for (const { id } of recipients) {
      ...
      if (payload.via_email && m.email) await sendEmail(m.email, payload.title, `<p>${payload.message}</p>`);
      if (payload.via_sms && m.contact_primary) await sendSms(m.contact_primary, payload.message);
    }
...
async function getRecipientIds(payload) {
