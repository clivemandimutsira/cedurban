  ...
  try {
    ...
    for (const member of missingMembers) {
      ...
      if (existingUser.length > 0) {
        ...
        createdUsers++;
        console.log(`[✔] Created user for ${email} with temp password: ${tempPassword}`);
      }
...
const populateFromMembers = async () => {
  try {
    // First, provision users for missing members and sync user_ids
...
const updateNewConverts = async () => {
  try {
    ...
    console.log(`[✔] New Converts updated - Promoted: ${promoted}, Inactivated: ${inactivated}, Archived: ${archived}`);
    return { promoted, inactivated, archived };
  } catch (err) {
